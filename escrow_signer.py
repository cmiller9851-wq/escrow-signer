...
def notify(msg): 
    ...
    if TELEGRAM_TOKEN and TELEGRAM_CHAT: 
        try: requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage", data={"chat_id": TELEGRAM_CHAT, "text": msg}, timeout=10) 
        except: pass
...
def main():
    if not all([RPC_URL, PRIVATE_KEY, ESCROW_CONTRACT, RECIPIENT]):
    ...
    if not w3.is_connected(): notify("RPC connection failed"); sys.exit(1)
    ...
    if DRY_RUN:
        notify(f"DRY RUN: Would release {amount_eth} ETH")
        return

    tx = contract.functions.releaseFunds(Web3.to_checksum_address(RECIPIENT), amount_wei).build_transaction({
        "chainId": CHAIN_ID, "nonce": w3.eth.get_transaction_count(account.address),
        "gas": GAS_LIMIT, "gasPrice": w3.to_wei(GAS_PRICE_GWEI, "gwei")
    })
    signed = w3.eth.account.sign_transaction(tx, pk)
    receipt = send_tx(w3, signed)

    url = f"https://etherscan.io/tx/{receipt.transactionHash.hex()}" if CHAIN_ID == 1 else f"https://sepolia.etherscan.io/tx/{receipt.transactionHash.hex()}"
    if receipt.status == 1:
